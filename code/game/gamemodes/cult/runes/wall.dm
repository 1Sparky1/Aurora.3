...
/datum/rune/wall/do_rune_action(mob/living/user, atom/movable/A)
	...
	if(A.density)
		to_chat(user, SPAN_CULT("You touch the rune, and you feel that the very space over the rune thickens."))
		parent.filters = filter(type="blur", size = 5)
	else
		to_chat(user, SPAN_CULT("You touch the rune, and you feel as the rune releases its grasp on space."))
