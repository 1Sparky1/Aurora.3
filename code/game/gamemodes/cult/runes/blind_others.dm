...
/datum/rune/blind/proc/do_blind(mob/living/user, atom/movable/A, var/range = 7, var/effect_mod = 1, var/special_effects = TRUE)
	...
	for(var/mob/living/carbon/C in viewers(get_turf(A), range))
		...
		to_chat(C, SPAN_DANGER("Suddenly you see a bright flash that blinds you!"))
		affected += C
	if(length(affected))
		user.say("Sti'kaliesin!")
		to_chat(user, SPAN_WARNING("The rune flashes, blinding those who not follow [cult.deity], and dissipates into fine dust."))
